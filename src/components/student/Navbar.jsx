...
const Navbar = () => {
  ...
  return (
    ...
    >
      {/* Logo */}
      <img onClick={()=>navigate('/')} src={assets.logo} alt="logo" className="w-24 lg:w-32 cursor-pointer" />
      ...
      <div className="hidden md:flex items-center gap-4">
        <button onClick={() => navigate('/educator')} className="text-gray-700 hover:text-blue-600">
  {isEducator ? 'Educator Dashboard' : 'Become Educator'}
      ...
      {menuOpen && (
        <div className="absolute top-16 left-0 w-full bg-white shadow-md md:hidden flex flex-col items-center gap-4 py-6 text-gray-700 z-50">
          {!isCourseListPage && <Link to="/courses" onClick={() => setMenuOpen(false)}>Courses</Link>}
          <Link to="/" onClick={() => setMenuOpen(false)}>Home</Link>
          <Link to="/contact" onClick={() => setMenuOpen(false)}>Contacts</Link>
          <Link to="/enroll" onClick={() => setMenuOpen(false)}>My Enrollments</Link>
